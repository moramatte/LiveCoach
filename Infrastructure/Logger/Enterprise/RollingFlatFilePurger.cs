...
namespace Infrastructure.Logger.Enterprise
{
	public class RollingFlatFilePurger
	{
		...
		private readonly int minAgeDays;
		...
		public RollingFlatFilePurger(string directory, string baseFileName, int cap, int minAgeDays, DateTimeProvider dateTimeProvider)
		{
		...
		public void Purge()
		{
			...
			using (var enumerator = sortedArchiveFiles.GetEnumerator())
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (Math.Abs((dateTimeProvider.CurrentUtcDateTime - enumerator.Current.CreationTimeUtc).TotalDays) <= minAgeDays)
						continue;
		...
		internal class ArchiveFile : IComparable<ArchiveFile>
		{
			...
			public ArchiveFile(string path)
			{
				...
				this.creationTimeUtc = GetCreationTimeUtc(path);
			}
			...
			{
				var creationDateComparison = this.CreationTimeUtc.CompareTo(other.CreationTimeUtc);
